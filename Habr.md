...
Вся механика робота распечатана на 3D-принтере. Я использовал OpenSCAD для создания параметрических моделей — это позволяет легко менять размеры колес или шасси, просто поменяв пару переменных в коде.

Основные компоненты:

-   **Шасси (`Шасси для робота v3`)**: Основа всей конструкции, куда крепятся моторы и электроника.
-   **Колеса (`Parametric_wheel`, `Rear_wheel`)**: Печатаются из твердого пластика (PLA/PETG).
-   **Шины (`Tire`, `Tire_protector`)**: Для сцепления лучше печатать из флекс-пластика (TPU), но можно и просто натянуть резинки.
-   **Кабина (`Cabin`)**: Корпус, прикрывающий "внутренности".
-   **Платформа для головы (`platform_for_head`)**: Крепление для камеры и матрицы.

Все исходники моделей (`.scad` и `.stl`) лежат в папке `3d`, так что вы можете легко модифицировать робота под свои нужды — например, приделать ему руки или снегоуборочный ковш.
